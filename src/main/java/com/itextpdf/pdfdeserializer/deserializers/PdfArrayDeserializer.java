...
public class PdfArrayDeserializer implements IPdfObjectDeserializer<PdfArray> {
    @Override
    public PdfArray deserialize(PdfTokenizer tokens, DeserializationContext context, boolean readAsDirectObject)
            throws IOException {
        ...
        while (true) {
            ...
            PdfObject obj = pdfObjectDeserializer.deserialize(tokens, context, true);
            if (obj == null) {
                if (tokens.getTokenType() != PdfTokenizer.TokenType.EndArray) {
                    processArrayReadError(tokens, context.getStrictnessLevel());
                }
                break;
            }
            array.add(obj);
    ...
    private void processArrayReadError(PdfTokenizer tokens, StrictnessLevel strictnessLevel) {
        ...
        if (StrictnessLevel.CONSERVATIVE.isStricter(strictnessLevel)) {
