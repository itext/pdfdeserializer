...
public class PdfNumberDeserializerTest {

    @Test
    public void testNumber() throws IOException {
        RandomAccessFileOrArray randomAccess = new RandomAccessFileOrArray(
                new RandomAccessSourceFactory().createSource(ByteUtils.getIsoBytes(9))
        );

        try (PdfTokenizer tokenizer = new PdfTokenizer(randomAccess)) {
            tokenizer.nextToken();
            PdfNumber actual = new PdfNumberDeserializer().deserialize(tokenizer, new DeserializationContext(), true);
            PdfNumber expected = new PdfNumber(9);
